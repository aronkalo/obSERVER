...
namespace obServer.GameLogic
{
    public sealed class ClientLogic : BaseLogic
    {
        public ClientLogic(IObServerModel model)
        {
            ...
            this.model = model;
            model.LoadItems("Map.xml");
            servers = new List<string>();
        }

        private List<string> servers;

        public string[] SearchServers(string name)
        {
            gameClient.Send(Operation.Connect, $"{name}");
            Thread.Sleep(100);
        ...
        {
            string serverName = request.Parameters.Split(';')[1];
            string serverAddr = request.Sender.ToString();
            string content = serverName + ";" + serverAddr;
            if (!servers.Contains(content))
            {
                servers.Add(content);
            }
        }
