...
namespace obServer.GameModel
{
    public sealed class ObServerModel :IObServerModel
    {
        ...
        public ObServerModel()
        {
            ...
            IWeapon weap = new Weapon(new EllipseGeometry() { RadiusX = 1, RadiusY = 1 }, Guid.NewGuid(), new double[] { player.Position.X + 10, player.Position.Y + 35 }, player.Rotation, false, 7000, 4, 500, 10, 0.005);
            ConstructItem(weap);
        ...
        {
            Items.RemoveAll(x => x.GetType() == typeof(StaticItem) && !x.Impact);
        }
    }
}
